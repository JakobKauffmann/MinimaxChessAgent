...
DEFAULT_TURN: Literal['white', 'black'] = 'white'

# Game state checker
class Board:


    def __init__(self, display: pygame.surface.Surface, width:float = 600, height:float = 600, config=None, turn=None):
        ...
        self.turn: Literal['white','black'] = turn if turn is not None else DEFAULT_TURN
        self.config = config if config is not None else DEFAULT_CONFIG
        self.squares: list[Square] = self.generate_squares()
        self.setup_board()

    @classmethod
    def without_display(cls, width:float = 600, height:float = 600, config=None, turn=None):
        return cls(None, width, height, config, turn)
    ...
    def get_config(self):
        current_config = [['' for x in range(8)] for y in range(8)]
        for y in range(8):
            for x in range(8):
                square = self.get_square_from_pos((x, y))
                if square.occupying_piece is None:
                    ...
                    color_prefix = 'w' if square.occupying_piece.color == 'white' else 'b'
                    current_config[y][x] = color_prefix + square.occupying_piece.notation
        return current_config

    def get_all_possible_moves(self, color: Literal['white', 'black']):
        moves = []
        for square in self.squares:
            if square.occupying_piece is not None:
                if square.occupying_piece.get_moves(self):
                    if square.occupying_piece.color == color:
                        squareMoves = square.occupying_piece.get_moves(self)
                        for i in squareMoves:
                            move = [square.pos,i.pos]
                            moves.append(move)
        return moves